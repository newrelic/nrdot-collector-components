...
* `nrdotplustcolRunner` - Configures, starts and stops one or more instances of nrdotplustcol which will be the subject of testing being executed.
  * `ChildProcess` - Implementation of `nrdotplustcolRunner` runs a single nrdotplustcol as a child process on the same machine as the test executor.
  * `InProcessCollector` - Implementation of `nrdotplustcolRunner` runs a single nrdotplustcol as a go routine within the same process as the test executor.
...
* `Testing` - This part may vary from what kind of testing developers would like to do. In existing implementation, we can refer to [End-to-End testing](https://github.com/newrelic/nrdot-plus-collector-components/blob/main/testbed/tests/e2e_test.go), [Metrics testing](https://github.com/newrelic/nrdot-plus-collector-components/blob/main/testbed/tests/metric_test.go), [Traces testing](https://github.com/newrelic/nrdot-plus-collector-components/blob/main/testbed/tests/trace_test.go), [Correctness Traces testing](https://github.com/newrelic/nrdot-plus-collector-components/blob/main/testbed/correctnesstests/traces/correctness_test.go), and [Correctness Metrics testing](https://github.com/newrelic/nrdot-plus-collector-components/blob/main/testbed/correctnesstests/metrics/metrics_correctness_test.go). For instance, if developers would like to design a trace test for a new exporter and receiver:
