...
func TestFilterSummaryMetricProperties(t *testing.T) {
	...
	t.Run("Resource count metric properties", func(t *testing.T) {
		...
		assert.Positive(t, includedDP.Timestamp())
		assert.Positive(t, filteredDP.Timestamp())
	})
	...
	t.Run("Threshold triggers metric properties", func(t *testing.T) {
		...
		for i := 0; i < gauge.DataPoints().Len(); i++ {
			...
			assert.Positive(t, dp.Timestamp())
		}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.shouldHaveTriggersMetric {
				...
				for i := 0; i < gauge.DataPoints().Len(); i++ {
					dp := gauge.DataPoints().At(i)
					assert.Positive(t, dp.IntValue(), "All data points should have positive values")
				}
			} else {
...
func validateThresholdTriggersMetric(t *testing.T, metric *pmetric.Metric, expectedStageHits map[string]int) {
	...
	for i := 0; i < gauge.DataPoints().Len(); i++ {
		...
		assert.Positive(t, dp.Timestamp(), "Should have timestamp")
		assert.Positive(t, dp.IntValue(), "Should only include stages with positive counts")
	}
