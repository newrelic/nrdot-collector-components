...
func TestMetricsBuilder(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			defaultMetricsCount++
			allMetricsCount++
			mb.RecordSqlserverBlockingQueryDetailsDataPoint(ts, 1, "query_type-val", 10, 10, "request_start_time-val", "query_id-val", 19, "blocking_query_text-val", "blocking_nr_service_guid-val", "blocking_normalised_sql_hash-val", "newrelic.event.type-val")
			...
			defaultMetricsCount++
			allMetricsCount++
			mb.RecordSqlserverSlowqueryHistoricalAvgRowsDataPoint(ts, 1, "query_id-val", "database_name-val")

			defaultMetricsCount++
			allMetricsCount++
			mb.RecordSqlserverSlowqueryHistoricalAvgWorkerTimeMsDataPoint(ts, 1, "query_id-val", "database_name-val")
			...
			defaultMetricsCount++
			allMetricsCount++
			mb.RecordSqlserverSlowqueryQueryDetailsDataPoint(ts, 1, "query_type-val", "query_id-val", "database_name-val", "plan_handle-val", "query_text-val", "collection_timestamp-val", "last_execution_timestamp-val", "normalised_sql_hash-val", "nr_service_guid-val", "newrelic.event.type-val")
			...
			for i := 0; i < ms.Len(); i++ {
				...
				case "sqlserver.activequery.query_details":
					assert.False(t, validatedMetrics["sqlserver.activequery.query_details"], "Found a duplicate in the metrics slice: sqlserver.activequery.query_details")
					validatedMetrics["sqlserver.activequery.query_details"] = true
					assert.Equal(t, pmetric.MetricTypeGauge, ms.At(i).Type())
					assert.Equal(t, 1, ms.At(i).Gauge().DataPoints().Len())
					assert.Equal(t, "Active query details including full query text and wait information (emitted as custom events)", ms.At(i).Description())
				...
				case "sqlserver.slowquery.historical_avg_elapsed_time_ms":
					...
					assert.True(t, ok)
					assert.Equal(t, "database_name-val", attrVal.Str())
				case "sqlserver.slowquery.historical_avg_rows":
					assert.False(t, validatedMetrics["sqlserver.slowquery.historical_avg_rows"], "Found a duplicate in the metrics slice: sqlserver.slowquery.historical_avg_rows")
					validatedMetrics["sqlserver.slowquery.historical_avg_rows"] = true
					assert.Equal(t, pmetric.MetricTypeGauge, ms.At(i).Type())
					assert.Equal(t, 1, ms.At(i).Gauge().DataPoints().Len())
					assert.Equal(t, "Historical average rows returned per execution (cumulative since plan cached)", ms.At(i).Description())
					...
					assert.True(t, ok)
					assert.Equal(t, "database_name-val", attrVal.Str())
				case "sqlserver.slowquery.historical_avg_worker_time_ms":
					assert.False(t, validatedMetrics["sqlserver.slowquery.historical_avg_worker_time_ms"], "Found a duplicate in the metrics slice: sqlserver.slowquery.historical_avg_worker_time_ms")
					validatedMetrics["sqlserver.slowquery.historical_avg_worker_time_ms"] = true
					assert.Equal(t, pmetric.MetricTypeGauge, ms.At(i).Type())
					assert.Equal(t, 1, ms.At(i).Gauge().DataPoints().Len())
					assert.Equal(t, "Historical average worker (CPU) time per execution in milliseconds (cumulative since plan cached)", ms.At(i).Description())
					assert.Equal(t, "ms", ms.At(i).Unit())
					dp := ms.At(i).Gauge().DataPoints().At(0)
					assert.Equal(t, start, dp.StartTimestamp())
					assert.Equal(t, ts, dp.Timestamp())
